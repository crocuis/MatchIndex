...
async function main() {
  ...
  const moduleUrl = new URL('../src/data/apiFootballCompetitionMaterialize.ts', import.meta.url);
  const { materializeApiFootballCompetitions } = await import(moduleUrl.href);
  ...
  if (args.has('--help') || args.has('-h')) {
    console.log(`Usage: node --experimental-strip-types scripts/api-football-materialize-competitions.mts [season ...] [options]

Options:
  --competitions=PL,FAC,PD,CDR,BL1,DFP,FL1,CDF,SA,CI,CL,EL
                        Restrict to supported competition codes
  --write               Upsert canonical competition/team/match records (default: dry-run)
...
`);
  ...
  const summary = await materializeApiFootballCompetitions({
