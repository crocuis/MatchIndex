...
async function main() {
  ...
  loadProjectEnv();
  const moduleUrl = new URL('../src/data/statsbombIngest.ts', import.meta.url);
  const { ingestStatsBombManifests } = await import(moduleUrl.href);
  const args = new Set(process.argv.slice(2));
  const orderedArgs = process.argv.slice(2).filter((arg) => !arg.startsWith('--'));
  const dryRun = args.has('--write') ? false : true;

  const summary = await ingestStatsBombManifests({
    dryRun,
