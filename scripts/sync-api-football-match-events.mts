...
const PROFILES: Record<CliOptions['profile'], { daysBack: number; description: string }> = {
  hourly: {
    daysBack: 0,
    description: '당일 API-Football 경기 이벤트 증분 수집',
  },
  daily: {
    daysBack: 1,
    description: '전일+당일 API-Football 경기 이벤트 재동기화',
  },
};
...
function printHelp() {
  console.log(`Usage: node --experimental-strip-types scripts/sync-api-football-match-events.mts [options]
...
Profiles:
  hourly  Same-day API-Football event sync
  daily   Re-sync yesterday and today for API-Football events
...
async function main() {
  ...
  const moduleUrl = new URL('../src/data/apiFootballMatchEventsSync.ts', import.meta.url);
  const { syncApiFootballMatchEvents } = await import(moduleUrl.href);
  const summary = await syncApiFootballMatchEvents({
