...
BEGIN
    ...
    FOREACH alias_slug IN ARRAY ARRAY['liverpool', 'liverpool-england'] LOOP
        ...
        VALUES ('team', canonical_id, alias_slug, NULL, 'historical', FALSE, 'approved', 'historical_rule', '015_remerge_liverpool_team_aliases')
        ON CONFLICT (entity_type, entity_id, alias_normalized)
        DO NOTHING;

        INSERT INTO entity_aliases (entity_type, entity_id, alias, locale, alias_kind, is_primary, status, source_type, source_ref)
        SELECT 'team', canonical_id, tt.name, tt.locale, 'common', FALSE, 'pending', 'merge_derived', '015_remerge_liverpool_team_aliases'
        ...
        IF to_regclass('public.match_lineups') IS NOT NULL THEN
            DELETE FROM match_lineups AS alias_ml
            USING match_lineups AS canonical_ml
            WHERE alias_ml.team_id = alias_id
              AND canonical_ml.team_id = canonical_id
              AND canonical_ml.match_id = alias_ml.match_id
              AND canonical_ml.match_date = alias_ml.match_date
        ...
        IF to_regclass('public.match_stats') IS NOT NULL THEN
            DELETE FROM match_stats AS alias_ms
            USING match_stats AS canonical_ms
            WHERE alias_ms.team_id = alias_id
              AND canonical_ms.team_id = canonical_id
              AND canonical_ms.match_id = alias_ms.match_id
              AND canonical_ms.match_date = alias_ms.match_date;
