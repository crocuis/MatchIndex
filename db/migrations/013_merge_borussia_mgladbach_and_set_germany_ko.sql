...
BEGIN
    ...
    FROM teams
    WHERE slug = 'borussia-mgladbach';

    SELECT id INTO canonical_id
    FROM teams
    WHERE slug = 'borussia-mo-nchengladbach-germany';

    IF alias_id IS NOT NULL AND canonical_id IS NOT NULL AND alias_id <> canonical_id THEN
        ...
        VALUES ('team', canonical_id, 'borussia-mgladbach', NULL, 'historical', FALSE, 'approved', 'historical_rule', '013_merge_borussia_mgladbach_and_set_germany_ko')
        ON CONFLICT (entity_type, entity_id, alias_normalized)
        DO NOTHING;

        INSERT INTO entity_aliases (entity_type, entity_id, alias, locale, alias_kind, is_primary, status, source_type, source_ref)
        SELECT 'team', canonical_id, tt.name, tt.locale, 'common', FALSE, 'pending', 'merge_derived', '013_merge_borussia_mgladbach_and_set_germany_ko'
